...
export function InlineCreateMarketForm({
  ...
  const [selectedTokenId, setSelectedTokenId] = useState<bigint | undefined>(dealerTokenIds?.[0]);
  // Sync when dealerTokenIds loads asynchronously after mount
  useEffect(() => {
    if (selectedTokenId === undefined && dealerTokenIds && dealerTokenIds.length > 0) {
      setSelectedTokenId(dealerTokenIds[0]);
    }
  ...
  const [positiveTeamSide, setPositiveTeamSide] = useState<'home' | 'away'>('home');
  ...
  const otherTeamName = positiveTeamSide === 'home' ? awayTeamName : homeTeamName;
  ...
  const autoTitle = (() => {
    if (variant === 'race')
      return t('inlineCreateMarket.titleOnDate', { team: homeTeamName, date: shortDateStr });
  ...
  const effectiveDescription = descriptionTouched && description ? description : autoDescription;
  ...
  const startPolling = () => {
    ...
    const cached = queryClient.getQueryData<{ id: string }[]>(['gameMarkets', sportCode, gameId]);
    ...
    pollRef.current = setInterval(async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['gameMarkets', sportCode, gameId] });
      ...
      if (newCount > prevMarketCountRef.current) {
        ...
        const newMarket = updatedArr.find(m => !prevMarketIdsRef.current.has(m.id));
        if (newMarket && pendingOracleConfigRef.current && indexerClient) {
          try {
            await indexerClient.setMarketOracleConfig(newMarket.id, pendingOracleConfigRef.current);
          } catch (e) {
  ...
  const isDeadlineValid = deadlineDate.getTime() - now.getTime() >= minDeadlineMs;
  ...
  const handleSubmit = async () => {
    ...
    const selectedTeamId = positiveTeamSide === 'home' ? homeTeamId : awayTeamId;
    ...
    <div className="rounded-lg border border-dashed border-primary/40 bg-primary/5">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">{t('markets.createMarket', 'Create Market')}</span>
        </div>
        <svg
      ...
      <AnimatedCollapse open={isExpanded}>
        <div className="px-4 pb-4 space-y-3">
            ...
            {variant === 'team-sport' && (
              <div className="md:w-44">
                ...
                <Select
                  ...
                  onValueChange={val => setPositiveTeamSide(val as 'home' | 'away')}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            {marketType === 'winloss' && (
              <div className="md:w-36">
                ...
                <Select
                  ...
                  onValueChange={val => setWinLossOutcome(val as WinLossOutcome)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="win">{t('market.condition.win', 'Win')}</SelectItem>
            ...
            {marketType === 'matchscore' && (
              <>
                ...
                <div className="md:w-24">
                  ...
                  <Select value={scoreOperator} onValueChange={setScoreOperator}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <div className="flex-1 min-w-0">
              <div className={`p-2.5 rounded-lg ${ui.background.muted}/50 text-sm`}>
                <span className="text-muted-foreground">{t('inlineCreateMarket.condition')} </span>
